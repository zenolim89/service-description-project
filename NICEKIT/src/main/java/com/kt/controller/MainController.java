...
public class MainController extends BaseController {
	 ...
	 * @param id 아이디
	 * @param pw 비밀번호
	 * @exception Case01 ID 불일치(NOT_FOUND_ACCOUNT) Case02 PW 불일치(NOT_MATCH_PASSWORD)
	 */
	 ...
	 * 
	 * @return
	 * @exception Case01 도메인 목록이 없을 경우(NOT_FOUND_DOMAIN)
	 */
	 ...
	 * 
	 * @return
	 * @exception Case01 연동규격 목록이 없을 경우(NOT_FOUND_SPEC)
	 */
	 ...
	 * 
	 * @return
	 * @exception Case01 템플릿 목록이 없을 경우(NOT_FOUND_TEMPLATE)
	 */
	/*
	 * @RequestMapping(value = "/getTemplate", method = RequestMethod.GET) public
	 * ResponseData<ResGetTemplate> getTemplate(@RequestParam String domainName) {
	 * ResGetTemplate result = new ResGetTemplate();
	 * 
	 * List<String> templateList = coreSvc.getTemplateList(domainName); if
	 * (templateList == null || templateList.isEmpty()) { throw new
	 ...
	 * @param domianName
	 * @return
	 * @exception Case01 등록된 사업장 목록이 없을 경우(NOT_FOUND_VENDOR)
	 */
	 ...
	 * @param domainName
	 * @return
	 * @exception Case01 편집할 사업장이 없을 경우(NOT_FOUND_TEMP)
	 */
	 ...
	 * @return
	 * @exception Case01 해당 사업장에 대한 미리보기가 존재하지 않는 경우(NOT_FOUND_PREVIEW_VENDOR)
	 * 
	 */
	 ...
	 * @param templateName
	 * @return
	 * @exception Case01 해당 템플릿에 대한 미리보기가 존재하지 않는 경우(NOT_FOUND_PREVIEW_TEMPLATE)
	 */
	 ...
	 * @param urlPath
	 * @return
	 * @exception Case01 이미 존재한 디렉토리가 있는 경우(DIRECTORY_ALREADY_EXISTS) Case02 디렉토리를
	 *                   찾지 못할 경우(NOT_FOUND_DIRECTORY) Case03 데이터베이스 INSERT
	 *                   Failed(DB_INSERT_FAILED) Case04 이미 규격이 존재 하는
	 *                   경우(SPEC_ALREADY_EXISTS)
	 * 
	 */
	...
	public ResponseData<ResCreateWithVendor> createWithVendor(@RequestBody ReqCreateWithVendor req) {
		...
		String path = coreSvc.createWithVendor(req.getDomianName(), req.getVendor(), req.getUrlPath(), req.getComUrl(),
				req.getTestUrl());
		result.setUrlPath(path);
	 ...
	 * @return
	 * @exception Case01 디렉토리가 이미 존재하는 경우(DIRECTORY_ALREADY_EXISTS) Case02 복사할 원본
	 *                   디렉토리가 존재하지 않는 경우(NOT_FOUND_DIRECTORY) Case03 데이터베이트 Insert
	 *                   오류(DB_INSERT_FAILED)
	 */
	...
	public ResponseData<ResCreateWithTemplate> createWithTemplate(@RequestBody ReqCreateWithTemplate req) {
		...
		String path = coreSvc.createWithTemplate(req.getDomainName(), req.getVendorName(), req.getUrlPath(),
				req.getSpecName());
	 ...
	 * @param vendorName 신규사업장명
	 * @return
	 * @exception Case01 저장 할 사업장 정보를 찾을수 없는 경우(NOT_FOUND_SAVE_TEMP)
	 */
	 ...
	 * @param vendorName 신규사업장명
	 * @return
	 * @exception Case01 배포 할 사업장 정보를 찾을 수 없는 경우(NOT_FOUND_DEPLOY_VENDOR) Case02 해당
	 *                   사업장의 디렉토리가 이미 존재하는 경우(DIRECTORY_ALREADY_EXISTS) Case03 복사할
	 *                   원본 디렉토리를 찾을수 없는 경우(NOT_FOUND_DIRECTORY)
	 */
