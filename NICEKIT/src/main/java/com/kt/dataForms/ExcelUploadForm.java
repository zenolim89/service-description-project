...
public class ExcelUploadForm {
	...
	/**
	 * 다음 데이터가 하나라도 없는 경우 return false, 모두 있는 경우 return true - transMethod,
	 * CommonURL, testURL, reqEx, resEx - header, reqParam, resParam
	 * 
	 * @return
	 */
	public Boolean isRegistered() {
		if (transMethod.isEmpty() || CommonURL.isEmpty() || testURL.isEmpty() || reqEx.isEmpty() || resEx.isEmpty())
			return false;
		if (header.isEmpty() || header == null)
			return false;
		if (reqParam.isEmpty() || reqParam == null)
			return false;
		if (resParam.isEmpty() || resParam == null)
			return false;
