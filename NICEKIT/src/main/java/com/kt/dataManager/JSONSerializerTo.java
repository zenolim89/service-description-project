...
public class JSONSerializerTo {
	...
	/**
	 * @author : "Minwoo Ryu" [2019. 3. 19. 오전 11:49:47] desc : response JOSN 객체를
	 *         만들기 위한 상위 DESCRIPTION. 모든 response는 해당 메소드 호출을 통하여 값을 정의하고, resData
	 *         JOSN 객체를 추가하여 반환
	 * @version :
	 * @param :
	 * @return : JSONObject
	...
	public JSONObject resultMsgforAuth(String id, String pw) {
		...
		if (authPw == null) {
			...
			// ArrayList<OwnServiceForm> ownList =
			// OwnServiceList.getInstance().getOwnList();
			...
			// //dialog
			// regiData.put("refDialog", this.getDialog(userList.get(j),
			// userList.get(j).getUserAuth()));
