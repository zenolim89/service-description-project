...
public class CoreService {
	 ...
	 * SelectDataTo selectTo = new SelectDataTo(); List<Row> rowList =
	 * selectTo.selectTemplateList(domainName, specName); List<String> list = new
	 * ArrayList<String>();
	 * 
	 * for (Row row : rowList) { list.add(row.getString("templatename")); }
	 * 
	 * return list; }
	 */
	 ...
	 * 
	 */
	public String createWithVendor(String domainName, String vendorName, String urlPath, String comUrl,
			String testUrl) {
		...
		if (specResult.equals("200")) {
			...
			if (createResult.equals("409")) {
				...
				if (resCode.equals("201")) {
					...
					url = "http://" + server.get("serverIp").toString() + ":" + server.get("port").toString()
