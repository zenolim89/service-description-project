...
function changeHeader(obj, field) {
	Utils.log(DEBUG, "changeHeader.call");
	Utils.log(DEBUG, "changeHeader.obj", obj);
	Utils.log(DEBUG, "changeHeader.field", field);
	Utils.log(DEBUG, "changeHeader.text", obj.value);
	
	var method = config["method"];
	var type = config["type"];
	var $elm = getParentElement().find("div.frm_tb").eq(0).find("ul").filter(":not(.hd)");
	var $targetElm = method == "GET" ? $("div[id=getDiv]").find("div[id=resDiv]").find("div.frm_tb").eq(0).find("ul").filter(":not(.hd)") : $("div[id=postDiv]").find("div[id=resDiv]").find("div.frm_tb").eq(0).find("ul").filter(":not(.hd)");
	try {
		var index = field == "key" ? $elm.find("li").find("input[name=key]").index($(obj)) : $elm.find("li").find("input[name=value]").index($(obj));
		if (field == "key") {
			$targetElm.find("li").find("input[name=key]").eq(index).val(obj.value);
		}
		else {
			$targetElm.find("li").find("input[name=value]").eq(index).val(obj.value);
		}
	}
	catch (err) {
		Utils.log(ERR, "changeHeader.err", err);
	}
	finally {
		$elm = null;
	}
...
function addHeader() {
	...
	var $targetElm = method == "GET" ? $("div[id=getDiv]").find("div[id=resDiv]").find("div.frm_tb").eq(0).find("ul").last() : $("div[id=postDiv]").find("div[id=resDiv]").find("div.frm_tb").eq(0).find("ul").last();
	...
	try {
		...
		setHtml.push('<ul>');
		setHtml.push('<li>');
		setHtml.push('<input class="text" type="text" name="key" placeholder="input key" title="Key" value="" onchange="changeHeader(this, \'key\');" />');
		setHtml.push('</li>');
		setHtml.push('<li>');
		setHtml.push('<input class="text" type="text" name="value" placeholder="input value" title="Value" value="" onchange="changeHeader(this, \'value\');" />');
		setHtml.push('</li>');
		...
		setHtml.push('<ul>');
		setHtml.push('<li>');
		setHtml.push('<input class="text" type="text" name="key" placeholder="input key" title="Key" value="" disabled="disabled" />');
		setHtml.push('</li>');
		setHtml.push('<li>');
		setHtml.push('<input class="text" type="text" name="value" placeholder="input value" title="Value" value="" disabled="disabled" />');
		setHtml.push('</li>');
		setHtml.push('<li class="ctrl">');
		// setHtml.push('<button type="button" class="bn btn_remove" onClick="removeHeader(\'' + method + '\', \'' + type + '\', this); return false;"><em>삭제</em></button>');
...
function removeHeader(obj) {
	...
	var $targetElm = method == "GET" ? $("div[id=getDiv]").find("div[id=resDiv]").find("div.frm_tb").eq(0).find("ul") : $("div[id=postDiv]").find("div[id=resDiv]").find("div.frm_tb").eq(0).find("ul");
	try {
		...
		if (len == 1 && index == 0) {
			...
			$targetElm.filter(":not(.hd)").eq(index).remove();
			addHeader();
		}
		else {
			$(obj).parents("ul").eq(0).remove();
