...
var ExcelParser = new function() {
			...
			if (!Utils.isEmpty(reqHeaders) && reqHeaders.length > 0) {
				for (var t = 0; t < reqHeaders.length; t++) {
					...
					if (headerStr.length < 2
								|| (Utils.isEmpty(headerStr[0]) || Utils.isEmpty(headerStr[1]))) {
						alert("request header " + Number(t + 1)
									+ "번째 행이 key, value 형식으로 입력되어 확인해 주세요.");
						return;
					}
				}
			}
			...
			if (!Utils.isEmpty(reqBodys) && reqBodys.length > 0) {
				if (method == "GET") {
					...
					for (var t = 0; t < reqBodys.length; t++) {
						...
						if (bodyStr.length < 3
									|| (Utils.isEmpty(bodyStr[0]) || Utils.isEmpty(bodyStr[1]) || Utils
												.isEmpty(bodyStr[0]))) {
							alert("request body " + Number(t + 1)
										+ "번째 행이  key, value, spec 형식으로 입력되어 확인해 주세요.");
							return;
						}
					}
				}
	...
	this.setExcelDomElement = function(excelJson) {
		...
		try {
			...
			ExcelParser.addHeader(method, "response", reqHeaders);
	...
	this.removeSpecDom = function(method, type) {
		...
		if (type == "request") {
			if (method == "GET") {
				$elm = $("div[id=getDiv]").find("div[id=reqDiv]").find("div.result").find(
							"div.frm_tb").filter(".spec_div");
			}
			else {
				$elm = $("div[id=postDiv]").find("div[id=reqDiv]").find("div.result").find(
							"div.frm_tb").filter(".spec_div");
			}
		}
		// response
			...
			if (method == "GET") {
				$elm = $("div[id=getDiv]").find("div[id=resDiv]").find("div.result").find(
							"div.frm_tb").filter(".spec_div");
			}
			else {
				$elm = $("div[id=postDiv]").find("div[id=resDiv]").find("div.result").find(
							"div.frm_tb").filter(".spec_div");
			}
		}
		...
		try {
			if (!Utils.isEmpty(headers) && headers.length > 0) {
				for (var i = 0; i < headers.length; i++) {
					...
					setHtml.push('<li>');
					setHtml
								.push('<input class="text" type="text" name="key" placeholder="input key" title="Key" value="'
											+ key + '" />');
					setHtml.push('</li>');
					setHtml.push('<li>');
					setHtml
								.push('<input class="text" type="text" name="value" placeholder="input value" title="Value" value="'
											+ value + '" />');
					setHtml.push('</li>');
				...
				for (var i = 0; i < bodys.length; i++) {
					...
					for (var t = 0; t < specObj["list"].length; t++) {
						if (specObj["list"][t] == spec) {
							setHtml.push('<option value="' + specObj["list"][t]
										+ '" selected="selected">' + specObj["list"][t]
										+ '</option>');
						}
						else {
							setHtml.push('<option value="' + specObj["list"][t] + '">'
										+ specObj["list"][t] + '</option>');
						}
					}
					setHtml.push('</select>');
					setHtml.push('</li>');
