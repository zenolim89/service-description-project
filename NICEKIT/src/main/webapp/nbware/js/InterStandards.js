...
function setScrollPosition(method, type, offset) {
	...
	getEditorId(method, type).setScrollPosition({
		scrollTop : offset
	});
}
...
function addHeader() {
	...
	try {
		...
		setHtml.push('<li>');
		setHtml
					.push('<input class="text" type="text" name="key" placeholder="input key" title="Key" />');
		setHtml.push('</li>');
		setHtml.push('<li>');
		setHtml
		...
		if (obj && obj.constructor === Array) {
			...
			if (objLen > 0) {
				for (var i = 0; i < objLen; i++) {
					...
					setHtml
								.push('<li style="width: 237px; text-align: left; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">');
